...
'  On Error GoTo catchError
  ...
  For line = 6 To endLine
    ...
    'タスクレベルが1ならリセット
    If mainSheet.Range(setVal("cell_LevelInfo") & line) = 1 Then
      parentTaskLine = ""
    ElseIf mainSheet.Range(setVal("cell_LevelInfo") & line) = "" Then
      GoTo Label_nextFor
    End If
    If mainSheet.Range(setVal("cell_LevelInfo") & line) < mainSheet.Range("B" & line + 1) And mainSheet.Range("B" & line + 1) <> "" Then
      endTaskLine = line + 1
      Do While mainSheet.Range(setVal("cell_LevelInfo") & line).Value <= mainSheet.Range("B" & endTaskLine).Value And mainSheet.Range("B" & endTaskLine) <> ""
        ...
        endTaskLine = endTaskLine + 1
      Loop
      If mainSheet.Range(setVal("cell_LevelInfo") & line).Value >= mainSheet.Range("B" & endTaskLine).Value Then
        endTaskLine = endTaskLine - 1
      End If
    ...
    If Range(setVal("cell_Assign") & line) = "工程" Then
      ...
      For lineP = line To 7 Step -1
        ...
        If Range(setVal("cell_Assign") & lineP) = "工程" And (Range(setVal("cell_LevelInfo") & line) > Range(setVal("cell_LevelInfo") & lineP)) Then
          mainSheet.Range(setVal("cell_TaskInfoP") & line) = Range(setVal("cell_LevelInfo") & lineP).row
          Exit For
        End If
      Next
    End If
...
'    Call Library.showDebugForm("タスクリスト確認", "　" & mainSheet.Range(setVal("cell_Info") & line))
    
    'Levelがなければループを抜ける
    If mainSheet.Range(setVal("cell_LevelInfo") & line) = "" Then Exit For
